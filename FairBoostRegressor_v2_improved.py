...
class FairBoostRegressor_v2_Improved(AdaBoostRegressor):
    """
    ...
    def _boost(self, iboost, X, y, sample_weight, random_state):
        ...
        """
        # Initialize target range on first iteration
        if iboost == 0:
            self.target_range_ = np.max(y) - np.min(y)
        ...
        # Calculate normalized error vector
        error_vect = np.abs(y_predict - y) / self.target_range_
        ...
        if self.protected != 'single':
            ...
            for k, v in grouped_metric.by_group.to_dict().items():
                tmp[' '.join(map(str, k))] = np.sqrt(v)
        else:
            tmp = {k: np.sqrt(v) for k, v in by_Z.items()}
        ...
        estimator_error = (masked_sample_weight * masked_error_vector).sum() * (1 - self.fairness_weight) + \
                          grouped_metric.difference() * self.fairness_weight
        ...
        # Calculate beta with numerical stability
        beta = np.clip(estimator_error / (1.0 - estimator_error), 1e-10, 1 - 1e-10)
        ...
        if not iboost == self.n_estimators - 1:
            sample_weight[sample_mask] *= np.power(
                beta,
                (1.0 - masked_error_vector) * self.learning_rate
            )
