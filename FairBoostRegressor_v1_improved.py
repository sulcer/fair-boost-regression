...
class FairBoostRegressor_v1_Improved(AdaBoostRegressor):
    """
    ...
    def _boost(self, iboost, X, y, sample_weight, random_state):
        ...
        if iboost == 0:
            ...
            self.group_weights = {str(group): 1.0 for group in np.unique(self.Z_tpl)}  # Group weights

        # Predict the target values
        y_predict = estimator.predict(X)

        # Calculate normalized errors
        errors = np.abs(y - y_predict) / self.target_range_

        # Calculate RMSE for this iteration
        rmse = np.sqrt(np.average(errors ** 2, weights=sample_weight))
        ...
        by_group_mae = groped_metric_mae.by_group.to_dict()

        # Handle single vs multiple protected attributes and store RMSE and MAE by group
        if self.protected != 'single':
            rmse_dict = {}
            mae_dict = {}
            for k, v in by_Z.items():
                group_key = ' '.join(map(str, k)) if isinstance(k, tuple) else str(k)
                rmse_dict[group_key] = np.sqrt(v)
                mae_dict[group_key] = by_group_mae[k]
        else:
            rmse_dict = {k: np.sqrt(v) for k, v in by_Z.items()}
            mae_dict = by_group_mae

        # Calculate relative error ratio
        max_rmse = max(rmse_dict.values())
        min_rmse = min(rmse_dict.values())
        rel_error_ratio = max_rmse / min_rmse if min_rmse > 0 else float('inf')

        # Store metrics for this iteration
        fairness_violation = grouped_metric.difference()
        self.grouped_metric_diff.append(fairness_violation)
        self.rmse_by_group.append(rmse_dict)
        self.mae_by_group.append(mae_dict)
        self.relative_error_ratio.append(rel_error_ratio)

        # Calculate combined error with fairness consideration
        estimator_error = rmse * (1 - self.fairness_weight) + fairness_violation * self.fairness_weight
        ...
        # Calculate estimator weight (inverse relationship with error)
        estimator_weight = self.learning_rate * (1.0 / estimator_error)

        # Update sample weights if not final iteration
        if not iboost == self.n_estimators - 1:
            # Calculate group-wise errors
            group_errors = {group: np.mean(errors[self.Z_tpl == group])
                            for group in np.unique(self.Z_tpl)}

            # Update group weights based on relative performance
            max_group_error = max(group_errors.values())
            for group, error in group_errors.items():
                self.group_weights[str(group)] = error / max_group_error if max_group_error > 0 else 1.0

            # Apply group weights to sample weights
            group_weights = np.array([self.group_weights[str(group)] for group in self.Z_tpl])
            updated_weights = errors * sample_weight * group_weights
